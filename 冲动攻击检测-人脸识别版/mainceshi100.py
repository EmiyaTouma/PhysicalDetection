...
MIN_FRAMES_HRV = 300
FPS = 10
TOTALTIME = 30  # 总的记录时长，方便测试时进行修改，正常为300s，测试时可改为30s/60s
...
class Heartbeat:
    def __init__(self, frames, q, con1, name):
        ...
        # SVM分类
        self.svmRF = joblib.load('model/svmRF.pkl')
        # 人脸检测网络
        self.model_face = Facenet()

        # 设置双端队列，并设置最大窗口
        self.bpm_avg_values = deque(maxlen=30 * FPS)  # 最多保存30s数据用于bpm计算
        self.hrv_avg_values = deque(maxlen=180 * FPS)  # 最多保存180s数据用于hrv计算
        self.br_avg_values = deque(maxlen=30 * FPS)  # 最多保存30s数据用于br计算
        self.SpO2_avg_values_R = deque(maxlen=30 * FPS)  # 最多保存30s数据用于SpO2计算
        self.SpO2_avg_values_B = deque(maxlen=30 * FPS)
        self.emotion_window = deque(maxlen=180 * FPS)  # 最多保存180s表情数据
            ...
            if yunxing == 1:
                while True:
                    ...
                    if not frames.empty():
                        ...
                        if q.empty():
                            q.put([np.array(frame), emotion_mode, bpm, result, cxmode, self.time1, br, SpO2,
                                   np.array(graph1)])
    ...
    def xiaobo(self, x):
        data = x
        w = pywt.Wavelet('db12')
        [ca7, cd7, cd6, cd5, cd4, cd3, cd2, cd1] = pywt.wavedec(data, w, level=7)
        n = len(data)
        datarec = pywt.upcoef('d', cd7, 'db12', level=7, take=n) + pywt.upcoef('d', cd6, 'db12', level=6,
                                                                               take=n) + pywt.upcoef('d', cd5, 'db12',
                                                                                                     level=5, take=n)
        return datarec
...
class Td:
    def __init__(self, q, con):
        self.cap = None
        self.framenum = 0
        self.data_get(q, con)  # q传出图像，con传入状态

    def data_get(self, q, con):
        yunxing = 0
        while True:
            if not con.empty():
                yunxing = con.get()
...
class mwindow(QWidget, Ui_jiemian):
    ...
    def Disp(self):
        ...
        if cxmode == 1:
            ...
            self.score.setText('--')  # 得分
            djs = int(TOTALTIME - (self.output[5]) / FPS)
            # self.tishi.setText('正在检测，剩余时间为{}秒'.format(djs))
            self.xianshi.setText('正在检测')
            self.progressBar.setValue(int(((TOTALTIME - djs) / TOTALTIME) * 100))
            ...
            wave = qimage2ndarray.array2qimage(self.output[8])
